...
export function calculatePrice(order: PlacedOrder): PricedOrder {
  ...
  const priceLines: PriceDetails["lines"] = [
    ...individualPiecePrices,
    ...extrasPerCombination,
  ];
...
function getPricesPerPiece(
  placedOrder: PlacedOrder,
): Array<[keyof SharedOrderFields, number]> {
  return [
    ["frameFinish", 50],
    ["frameType", 90],
  ];
...
function getExtrasCombinationPrice(
  order: PlacedOrder,
): Array<[string, number]> {
